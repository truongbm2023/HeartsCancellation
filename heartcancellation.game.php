...
class heartcancellation extends Table
{
    ...
    */
    ...
    {    
        ...
        $default_color = array( "ff0000", "008000", "0000ff", "ffa500", "A52A2A", "9400D3", "696969", "26282A");

        $start_points = self::getGameStateValue( 'gameLength' ) == 1 ? 100 : 200;
        ...
        if($num_of_players == 5){ //if there are 5 players need to remove four cards

            foreach( $this->colors as  $color_id => $color ) // spade, heart, diamond, club
            {
            for( $value=2; $value<=14; $value++ )   //  2, 3, 4, ... K, A
            {
                if($value== 2 && $color_id == 4){ //remove dia 2

                }else if($value== 3 && $color_id == 4){ // remove dia 3


                }else{
                    $cards[] = array( 'type' => $color_id, 'type_arg' => $value, 'nbr' => 2);// ============== DECK RESIZING HERE, 2(hearts), 3(clubs)
                }
            }
        }

        }else if($num_of_players == 6){ //if there are 6 players need to remove 2 cards
            foreach( $this->colors as  $color_id => $color ) // spade, heart, diamond, club
            {
            for( $value=2; $value<=14; $value++ )   //  2, 3, 4, ... K, A
            {
        ...
        }else if($num_of_players == 7){ //if there are 7 players need to remove 6 cards
            foreach( $this->colors as  $color_id => $color ) // spade, heart, diamond, club
            {
            ...
            for( $value=2; $value<=14; $value++ )   //  2, 3, 4, ... K, A
            {
                ...
                }else if($value== 4 && $color_id == 4){// remove dia 4
            ...
            foreach( $this->colors as  $color_id => $color ) // spade, heart, diamond, club
            {
                for( $value=2; $value<=14; $value++ )   //  2, 3, 4, ... K, A
                {
                    $cards[] = array( 'type' => $color_id, 'type_arg' => $value, 'nbr' => 2);// ============== DECK RESIZING HERE
                }
            }
...
////////////    
    ...
    */
    ...
    function getPlayersToDirection()
    {
        ...
        if($num_of_players == 4){ // Checks nums of players and assigns num of directions
            $directions = array( 'S', 'W', 'N', 'E' );
        }else if($num_of_players == 5){
            $directions = array( 'S', 'W', 'N', 'E', 'NE' );
        }else if($num_of_players == 6){
            $directions = array( 'S', 'W', 'N', 'E', 'NE', 'NW' );
        }else if($num_of_players == 7){
            $directions = array( 'S', 'W', 'N', 'E', 'NE', 'NW', 'SE' );
        }else if($num_of_players == 8){
            $directions = array( 'S', 'W', 'N', 'E', 'NE', 'NW', 'SE', 'SW' );
        }
...
//////////// 
    ...
    */
    ...
    function playCard( $card_id )
    {
        ...
        $bFirstCard = ( count( $playerhands ) == 26 );
        ...
        if( $this->cards->countCardInLocation( 'hand' ) == 104 )
        {
...
////////////
    ...
    */
    ...
    function stNewHand() //SHUFFLE CARDS AND PASS THEM OUT
    {
        ...
        // Create deck, shuffle it and give 26 initial cards
        $players = self::loadPlayersBasicInfos();
        ...
        foreach( $players as $player_id => $player )
        {
            $cards = $this->cards->pickCards( $deal_cards, 'deck', $player_id );
    ...
    function stTakeCards()//================================== FINDING CLUBS OF 2 AND PASSING THREE CARDS CODE
    {
        ...
        foreach( $players as $player_id => $player){ //Goes through each player
            $cards = $this->cards->getCardsInLocation( 'hand', $player_id ); //Cards in hand
            foreach($cards as $cards){
                if($cards['type'] == 3 && $cards['type_arg']== 2){
                    $twoClubCardOwner = $cards['location_arg'];
                }
            }
    ...
    function stNextPlayer()
    {
        ...
        $players = self::loadPlayersBasicInfos();//loads player info
        $num_of_players = count( $players );//Count of num of players

        
        if( $this->cards->countCardInLocation( 'cardsontable' ) == $num_of_players ) //============================== CARD PAIRING
        {
            ...
            $org_value_player_id = null;
            $currentTrickColor = self::getGameStateValue( 'trickColor' );
            
            foreach( $cards_on_table as $card )
            {
                $org_value_player_id = $card['location_arg'];

                if( $card['type'] == $currentTrickColor )   // Note: type = card color
                {
                    $pairing = false; 
                    foreach($cards_on_table as $card2){//for loop to find pairing
                        if($card['type_arg'] == $card2['type_arg'] && ($card['location_arg'] != $card2['location_arg'])){
                            $pairing = true;
                        }
                    }

                    if( $best_value_player_id === null && $pairing == false)
                    {
                    ...
                    else if( $card['type_arg'] > $best_value && $pairing == false)
                    {
                ...
                if( $best_value_player_id === null ){//if all card pairred
                    //throw new feException( self::_("Error, nobody wins the trick") );

                // Active this player => he's the one who starts the next trick
                //cardspairred

               // echo "ALL CARDS PAIRRED";





                $this->cards->moveAllCardsInLocation( 'cardsontable', 'carspairred');
                $this->gamestate->changeActivePlayer(  $org_value_player_id );


                               //Notifications
                               self::notifyAllPlayers( 'trickWin', clienttranslate('Nobody wins! All cards played all pairred'), '' );    
                            
                            
                             self::notifyAllPlayers( 'pairredPile','', '' );
            ...
            else{//If all cards not pairred


            // Move all cards to "cardswon" of the given player
            if($this->cards->countCardInLocation( 'carspairred' ) != 0 ){
                $this->cards->moveAllCardsInLocation( 'carspairred', 'cardsontable');

                //Notifcations for pairred cards on table.
                self::notifyAllPlayers( 'trickWin', clienttranslate('${player_name} wins the trick and the table!!'), array(
                    'player_id' => $best_value_player_id,
                    'player_name' => $players[ $best_value_player_id ]['player_name']
                ) );  


                // self::notifyAllPlayers( 'pairredEmpty','', array(
                //     'player_id' => $best_value_player_id
                // ) );
            ...
            // self::notifyAllPlayers( 'deleteCards','', array(
            //     'player_id' => $best_value_player_id
            // ) );
    ...
    function stEndHand()
    {
        ...
        $player_with_queen_of_spades_arr = array();
        ...
        foreach( $cards as $card )
        {
            ...
            if( $card['type'] == 1 && $card['type_arg'] == 12 )    // Note: 1 = spade && 12 = queen
            {
                ...
                array_push($player_with_queen_of_spades_arr, $player_id);
            }
        ...
        $bOnePlayerGetsAll = ( $nbr_nonzero_score == 1 ); //scoring
        ...
        // Queen of spades
        $newRow = array( array( 'str' => clienttranslate('Queen of Spades'), 'args' => array() ) ); //Displaying Queen of Spades
        foreach( $player_to_hearts as $player_id => $hearts )
        {
        
            $counter = 0;
            if( $player_id == $player_with_queen_of_spades_arr[0] ||  $player_id == $player_with_queen_of_spades_arr[1])
                {
                    if ($player_with_queen_of_spades_arr[0] == $player_with_queen_of_spades_arr[1]) {
                        $counter++;
                    }
                    $counter++;
                    $newRow[] = $counter;
