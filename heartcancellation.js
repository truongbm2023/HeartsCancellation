...
function (dojo, declare) {
    return declare("bgagame.heartcancellation", ebg.core.gamegui, {
        constructor: function(){
            console.log('heartcancellation constructor');
        ...
        */        
        setup: function( gamedatas )
        {
            ...
            //for cards pairred
            // for( i in this.gamedatas.cardspairred ){
            //     var card = this.gamedatas.cardspairred[i];
            //     var color = card.type;
            //     var value = card.type_arg;
            //     var player_id = card.location_arg;
            //     this.cardspairred( player_id, color, value, card.id );
            // }
        ...
        */
        ...
        // cardspairred: function(player_id, color, value, card_id){
        //     //nothing for now
        // },
        ...
        */
        ...
        onPlayerHandSelectionChanged: function(  )
        {
            ...
            if( items.length > 0 )
            {
                if( this.checkAction( 'playCard', true ) )
                {
                    ...
                    this.ajaxcall( "/heartcancellation/heartcancellation/playCard.html", { 
                            id: card_id,
                            lock: true 
        ...
        onGiveCards: function()
        {
            if( this.checkAction( 'giveCards' ) )
            {
                ...
                this.ajaxcall( "/heartcancellation/heartcancellation/giveCards.html", { cards: to_give, lock: true }, this, function( result ) {
                }, function( is_error) { } );                
            }        
        },
        ...
        */
        ...
        setupNotifications: function()
        {
            ...
            dojo.subscribe( 'pairredPile', this, "notif_pairredPile");
            dojo.subscribe( 'pairredEmpty', this, "notif_pairredEmpty");
        ...
        //PAIRRED ANIMATION
        notif_deleteCards: function(nottif){
            for( var player_id in this.gamedatas.players )
            {
                var anim = this.slideToObject( 'cardontable_'+player_id, 'cardontable_'+player_id );
        ...
        notif_pairredEmpty: function( notif){
            var winner_id = notif.args.player_id;

            var anim = this.slideToObject( 'pairredtablecard', 'overall_player_board_'+ winner_id );
            dojo.connect( anim, 'onEnd', function( node ) { dojo.destroy(node);  } );
            anim.play();
        },


        notif_pairredPile: function( notif){

            //var winner_id = notif.args.player_id;
            for( var player_id in this.gamedatas.players )
            {
                var anim = this.slideToObject( 'cardontable_'+player_id,  'pairredtablecard' );
                dojo.connect( anim, 'onEnd', function( node ) { dojo.destroy(node);  } );
                anim.play();
            }
